# 第 26 周：DTO、Pipe、Guard、Interceptor 与错误

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-26`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 25 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

接口返回 500 和底层数据库错误，用户不知道如何处理，日志也无法关联请求。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

Pipe 转换校验输入，Guard 决定能否访问，Interceptor 包裹调用做计时/转换，Exception Filter 统一协议错误。错误包含稳定 code、用户 message 和 requestId，内部 cause 仅入受控日志。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：对应 Bean Validation、Security Filter、HandlerInterceptor 和 ControllerAdvice。
...
通过测试后，再查看 `solutions/week-26`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 DTO、Pipe、Guard、Interceptor 与错误 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 27 周](/course/week-27)。
