# 第 1 周：环境、REPL 与第一个程序

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-01`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 0 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

你接手一个云函数，却不知道代码由谁执行、从哪里开始。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

JavaScript 是语言规范；Node.js 是实现运行时；npm 是包管理与脚本入口。程序从模块顶层开始执行，函数只有被调用才进入栈。REPL 适合验证一个小假设，脚本适合保存可重复实验。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：JDK、JVM、Maven 与 Java 源码的关系，可类比语言、Node 和 npm，但 Node 不需要先生成 JVM 字节码才能运行普通脚本。
...
通过测试后，再查看 `solutions/week-01`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 环境、REPL 与第一个程序 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 2 周](/course/week-02)。
