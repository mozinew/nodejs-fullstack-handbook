...
**本周完成标准**：读完本课、完成 `labs/week-18`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 17 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

数据修复执行到一半超时，重跑后部分记录被重复追加。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

findStream 控制读取批次，batchUpdate 减少往返，长任务扩展时间窗口；三者解决不同问题。每批记录 checkpoint、成功数、失败业务键。写操作使用唯一约束、幂等键或状态机。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：类似 Spring Batch 的 chunk、JobRepository 和 restartability。
...
通过测试后，再查看 `solutions/week-18`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 流式、批量、长任务与幂等 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 19 周](/course/week-19)。
