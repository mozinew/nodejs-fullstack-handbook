# 第 23 周：React 数据请求、竞态与测试

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-23`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 22 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

用户快速切换周期，旧请求最后返回并覆盖新看板。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

请求状态至少包含 idle/loading/success/empty/error。用 AbortController 取消旧请求，或用递增 requestId 只接受最新响应。组件测试点击和可见结果，不读取 Hook 内部状态。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：类似后端请求版本控制，但竞态发生在一个页面的多个异步生命周期。
...
通过测试后，再查看 `solutions/week-23`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 React 数据请求、竞态与测试 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 24 周](/course/week-24)。
