# 第 9 周：fs、path、URL 与 process

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-09`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 8 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

本地函数运行器接受函数名并拼路径，若不校验会产生目录穿越。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

path.resolve/join 处理平台分隔符，但不能自动阻止越界；解析后还要确认目标位于允许根目录。process.env 的值都是字符串或 undefined。URL 类负责结构化解析，不能靠字符串 startsWith 完成安全校验。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：对应 Java NIO Path、System.getenv 与 URI，但 Node API 多为异步/回调和 Promise 变体。
...
通过测试后，再查看 `solutions/week-09`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 fs、path、URL 与 process 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 10 周](/course/week-10)。
