# 第 10 周：Buffer、Stream、EventEmitter 与背压

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-10`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 9 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

findStream 每批都 push 到 allRecords，最终仍因全量数据耗尽内存。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

Buffer 保存字节；编码决定字节与文本的转换。Stream 按块传输并通过背压协调生产消费速度。aPaaS findStream 只是分页回调，只有批内处理后丢弃数据才真正控制内存。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：对应 Java ByteBuffer/InputStream/Flow，但 Node 流与事件循环集成。
...
通过测试后，再查看 `solutions/week-10`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 Buffer、Stream、EventEmitter 与背压 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 11 周](/course/week-11)。
