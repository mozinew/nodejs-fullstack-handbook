# 第 11 周：HTTP、进程、Worker 与性能诊断

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-11`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 10 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

一个 CPU 密集报表让同一实例的所有 HTTP 请求卡住。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

HTTP 请求由方法、URL、头和体组成；响应必须有状态码、头和体。CPU 工作占据主线程时应拆给 Worker 或独立服务。先测事件循环延迟、CPU、内存和依赖耗时，再优化。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：Node 单进程不像传统 Servlet 线程池；一个阻塞回调就能拖慢该实例全部请求。
...
通过测试后，再查看 `solutions/week-11`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 HTTP、进程、Worker 与性能诊断 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 12 周](/course/week-12)。
