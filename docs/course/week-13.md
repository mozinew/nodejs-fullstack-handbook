...
**本周完成标准**：读完本课、完成 `labs/week-13`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 12 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

成功和失败返回混在一个可选字段对象里，调用方无法穷尽处理。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

泛型描述输入输出关系，不是“任何类型”。判别联合让 success/error 分支拥有不同字段，switch/default-never 可检查新增状态。映射类型适合机械变换，不应隐藏业务语义。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：可类比 Java 泛型和 sealed interface，但 TS 采用结构类型。
...
通过测试后，再查看 `solutions/week-13`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 泛型、高级类型与领域建模 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 14 周](/course/week-14)。
