# 第 7 周：Promise、async/await 与事件循环

<div class="learning-contract">

**本周完成标准**：读完本课、完成 `labs/week-07`、让测试通过，并能不用答案解释 CRM 源码中的对应机制。
...
前置知识：第 6 周内容。本周建议投入 10–15 小时，其中至少一半用于编码和测试。

## 从一个真实问题开始

批量保存使用 async forEach，函数已经返回成功，后台写入却仍未完成。

先写下你的判断：错误会在什么时候发生、谁能观察到、怎样用最小实验证明。课程的目标不是记答案，而是形成这套推理过程。

## 从零理解

async 函数总返回 Promise。await 暂停当前 async 函数并把后续安排为微任务，不会阻塞进程。forEach 不消费回调返回的 Promise；串行用 for...of，并行用 Promise.all。

把机制分成三层理解：语法告诉你代码怎样写；运行时决定它何时执行；工程约束决定它在失败、并发和变化下是否仍正确。只会第一层，代码通常只能在演示环境工作。

<div class="java-bridge">

**Java/Spring 对照**：await 不是 Thread.sleep，也不等于创建线程；更接近把 continuation 注册到 CompletionStage。
...
通过测试后，再查看 `solutions/week-07`。参考答案不是唯一实现；只要行为、错误边界和可读性满足测试即可。
...
完成本周后，你应能把 Promise、async/await 与事件循环 用在一个可运行程序中，并能在 CRM 项目里识别它的真实边界。
下一课：[第 8 周](/course/week-08)。
